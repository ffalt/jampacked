...
import { SeriesIndexScreen } from '../../src/screens/SeriesIndexScreen';
import { IndexList } from '../../src/components/IndexList';
import { ErrorView } from '../../src/components/ErrorView';
import { useLazySeriesIndexQuery } from '../../src/services/queries/seriesIndex';
...
jest.mock('../../src/services/queries/seriesIndex');
...
function setState(state: IndexState): void {
	jest.mocked(useLazySeriesIndexQuery).mockReturnValue([mockGetIndex, state] as never);
}

describe('SeriesIndexScreen', () => {
	beforeEach(() => {
	...
	it('fetches the index on mount (with context/route params where relevant)', async () => {
		await render(<SeriesIndexScreen {...screenProps(SeriesIndexScreen)} />);
	...
	it('passes the index, title, loading and reload to IndexList', async () => {
		...
		expect(properties?.title).toBe('Series');
